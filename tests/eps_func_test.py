...
while t < T:
    ...
    a_R = u * v * ufl.dx
    L_R = 1/dt * xi * v * ufl.dx - 1/dt * xi_prev * v * ufl.dx + ufl.dot(w, ufl.grad(xi)) * v * ufl.dx

    # Solve linear system
    problem = LinearProblem(a_R, L_R, petsc_options={"kst_type": "preonly", "pc_type": "lu"})
    R = problem.solve() # returns dolfinx.fem.Function
    print(R.x.array)
    ...
    for cell in range(num_cells):
        ...
        Rk = np.max(np.abs(R.x.array[loc2glb]))
