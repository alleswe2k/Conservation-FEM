""" Test of calculating residual with projection """
...
# One GFEM step for residual calculation
t += dt
# print(t)
...
for i in range(num_steps-1):
    ...
    bilinear_form_R = fem.form(a_R)
    linear_form_R = fem.form(L_R)

    A_R = assemble_matrix(bilinear_form, bcs=[bc])
    A_R.assemble()
    b_R = create_vector(linear_form)
    ...
    with b_R.localForm() as loc_b:
        loc_b.set(0)
    assemble_vector(b_R, linear_form)
    apply_lifting(b_R, [bilinear_form], [[bc]])
    b_R.ghostUpdate(addv=PETSc.InsertMode.ADD_VALUES, mode=PETSc.ScatterMode.REVERSE)
    set_bc(b_R, [bc])

    # Solve the linear system
    solver_R.solve(b, uh.x.petsc_vec)
    uh.x.scatter_forward()
