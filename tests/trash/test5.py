...
w_inf_norm = np.max(np.linalg.norm(w.x.array.reshape(-1, 2), axis=1))
...
# Time-stepping loop
while t < T:
    # Matrix-vector multiplications and residual R calculation
    ...
    R.array[:] /= R_norm if R_norm != 0 else 1
    ...
    for cell in range(num_cells):
        ...
        Bk = np.max(np.linalg.norm(w_values[loc2glb], axis=1))
    ...
    # Assemble residual-based viscosity term as a vector
    Rv_form = fem.form(Rv * ufl.inner(ufl.grad(xi), ufl.grad(v)) * ufl.dx)
    Rv_vec = assemble_vector(Rv_form)

    # Construct RHS vector L_vec
    L_vec = M_xi - C_xi - Rv_vec

    # Solve system A * xi = L_vec
