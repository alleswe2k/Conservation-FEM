...
for mesh_size in mesh_sizes:
    ...
    for i in tqdm(range(num_steps-1)):
        ...
        assert (converged)
        #RH.x.array[:] = RH.x.array / np.max(u_n.x.array - np.mean(u_n.x.array)) #---- 0.28545154999828526
        #RH.x.array[:] = rv.normalize_Rh_robust_ni(uh, RH,node_patches) #----0.28545154999828526
        # RH.x.array[:] = rv.normalize_Rh_robust_simple(uh, RH,node_patches) #-----0.28545154999828526
        # RH.x.array[:] = rv.normalize_Rh(uh, RH,node_patches) #----- 0.28545154999828526
        #epsilon = rv.get_epsilon_x2(uh, u_n, velocity_field, RH, h_CG, node_patches) #-----0.31978910126646165
        #epsilon = rv.get_epsilon_x3(uh, u_n, velocity_field, RH, h_CG, node_patches) #----0.31978910126646165

        epsilon = rv.get_epsilon_robust(uh, velocity_field, RH, h_CG,)
