...
# TODO: Patch this / make sure it's a valid alternative
# Project directly to V
# h_expr = ufl.CellDiameter(domain)
# v = ufl.TestFunction(V)
...
""" Test of node iteration """

# num_dofs = V.dofmap.index_map.size_local  # Total number of degrees of freedom (DOFs)

# # Loop over each DOF (node)
# for dof in range(num_dofs):
#     # Retrieve the global DOF index (use the adjacency list to map DOFs)
#     global_dof = V.dofmap.list[dof][0]  # Get the global DOF index (index 0 for the first DOF in the list)

#     # Get the coordinates of the node (DOF)
#     x = V.tabulate_dof_coordinates()[global_dof]  # Use the global DOF index

#     # Print or perform any other operation with the DOF coordinates
#     print(f"DOF {global_dof}: Coordinates = {x}")
