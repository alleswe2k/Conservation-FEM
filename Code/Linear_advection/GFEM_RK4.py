...
# u_temp = fem.Function(V)  # Temporary variable for RK stages

# for i in range(num_steps):
#     # Stage 1
#     b = create_vector(fem.form(-ufl.dot(w, ufl.grad(u_n)) * v * ufl.dx))
#     solver.solve(b, uh.x.petsc_vec)
#     uh.x.scatter_forward()
#     k1 = uh.x.array.copy()

#     # Stage 2
#     u_temp.x.array[:] = u_n.x.array + 0.5 * dt * k1
#     b = create_vector(fem.form(-ufl.dot(w, ufl.grad(u_temp)) * v * ufl.dx))
#     solver.solve(b, uh.x.petsc_vec)
#     uh.x.scatter_forward()
#     k2 = uh.x.array.copy()

#     # Stage 3
#     u_temp.x.array[:] = u_n.x.array + 0.5 * dt * k2
#     b = create_vector(fem.form(-ufl.dot(w, ufl.grad(u_temp)) * v * ufl.dx))
#     solver.solve(b, uh.x.petsc_vec)
#     uh.x.scatter_forward()
#     k3 = uh.x.array.copy()
...
#     # Stage 4
#     u_temp.x.array[:] = u_n.x.array + dt * k3
#     b = create_vector(fem.form(-ufl.dot(w, ufl.grad(u_temp)) * v * ufl.dx))
#     solver.solve(b, uh.x.petsc_vec)
#     uh.x.scatter_forward()
#     k4 = uh.x.array.copy()
