    ...
    for degree in degrees:
        ...
        for fraction in fractions:
            ...
            if degree == 1:
                ...
                CFL = 0.05 # Note: 0.02 for SI RK4 to work
            else:
                CFL = 0.02
            t = 0  # Start time
            T = 1.0  # Final time
            ...
            xdmf_sol = io.XDMFFile(domain.comm, f"{location_data}/sol_{"discont" if DISCONT else "cont"}_D{degree}_h{fraction}_alt.xdmf", "w")
            xdmf_eps = io.XDMFFile(domain.comm, f"{location_data}/eps_{"discont" if DISCONT else "cont"}_D{degree}_h{fraction}.xdmf", "w")
            xdmf_sol.write_mesh(domain)
            xdmf_eps.write_mesh(domain)
            print("FILENAME:", f"{location_data}/sol_{"discont" if DISCONT else "cont"}_D{degree}_h{fraction}_alt.xdmf")
        ...
        pde.plot_convergence(L2_errors, fractions, f"P{degree}", f"conv_{"discont" if DISCONT else "cont"}_D{degree}", location_figures)